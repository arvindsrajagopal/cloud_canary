...
The checked-in plan is active through the bounded failure-classification,
recovery-policy, and checkpoint-correction phase. It uses invariant-sized tasks
and stops deliberately at `R11`, which is marked as the next replan gate. The
...
Execution stops at the `R11` replan gate before entering the remaining broad
phases. Every Codex
