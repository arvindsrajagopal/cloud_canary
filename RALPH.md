...
```bash
.venv/bin/python scripts/ralph.py --max-iterations 36 --max-attempts 3
```
...
The four ordered stages are R01-R06 for core health and scheduling contracts,
R07-R12 for service state, recovery, and HTTP lifecycle, R13-R18 for secure
...
At R06, R12, R18, and R24, Ralph also builds a cumulative patch from the prior
...
the next prompt. Attempt counts persist across runner restarts; exhausting the
configured limit stops the loop until a human deliberately resolves or resets
the task state.

## Context and Quality Controls

The 119 regression criteria are divided into 24 tasks of no more than five
criteria each. Every Codex invocation is an ephemeral session. The runner
...
iteration exceeds 12 changed files or 1,200 diff lines. It saves the isolated
...
8,000 feedback characters and at most 20 blocking review findings.
...
Tasks `R06`, `R12`, `R18`, and `R24` are logical milestones. After their tests
